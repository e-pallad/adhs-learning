...
tests/
  ...
  integration/
    ...
    progress-course.test.ts   # POST /api/progress/course
...
No transaction rollback machinery — the per-test reset is simpler and sufficient.
...
| `awardDailyLoginXP` second call same day | idempotent — no double XP |
...
Each route test constructs a `NextRequest` and calls the handler directly. Verifies:
- Happy path: correct response shape + DB state after call
- Validation errors: 400 for invalid status, negative minutesSpent, name > 100 chars, unknown blockId
...
Uses `critical` threshold (not `high`) because the current high-severity findings are all inside Prisma 7's internal CLI tooling (`@prisma/dev`, `@hono/node-server`, `effect`) — not reachable from the app runtime. These cannot be fixed without a major version downgrade (`prisma@6.x`) that would break the entire Prisma 7 setup. The audit job will still surface any new critical vulnerabilities that appear.

**Known Prisma 7 CVEs (informational — not runtime-exploitable):**
- `GHSA-wc8c-qw6v-h7f6` — `@hono/node-server` auth bypass in Serve Static (Prisma Studio only)
- `GHSA-9r54-q6cx-xmh5` — `hono` XSS in ErrorBoundary (Prisma Studio only)
- `GHSA-38f7-945m-qr2g` — `effect` AsyncLocalStorage context contamination (Prisma RPC internal)
